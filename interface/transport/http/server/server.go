...
import (
	...
	"github.com/ashihara-api/geo/interface/transport/di"
	httprouter "github.com/ashihara-api/geo/interface/transport/http/router"
)
...
func newRouter(conf *Config) (mux *chi.Mux, err error) {
	...
	httprouter.NewPrefecture(di.Prefecture(dr, conf.Logger)).Route(mux)          // nolint: errcheck
	httprouter.NewCity(di.City(conf.HTTPClient, dr, dw, conf.Logger)).Route(mux) // nolint: errcheck
